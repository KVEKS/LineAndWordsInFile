...
public class LineAndWordsInFile {
    public static void main(String[] args) {
        ...
        if (!isDirectory(Paths.get(nameIn))) {
            try (BufferedReader reader = new BufferedReader(new FileReader(nameIn))) {
                String line = "";
            ...
            for (int i = 0; i < listOfFiles.length; i++) {
                try (BufferedReader reader = new BufferedReader(new FileReader(listOfFiles[i].getAbsolutePath()))) {
                    String line = "";
                    while((line = reader.readLine()) != null) {
                        countLines++;
                        String[] strings = line.split(" ");
